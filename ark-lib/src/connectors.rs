...
mod test {
    ...
    fn test_budget() {
        ...
        let size = chain
            .iter_signed_txs(&key)
            .map(|t| t.vsize() as u64)
            .sum::<u64>();
        assert_eq!(size, ConnectorChain::total_vsize(100));
        chain
            .iter_unsigned_txs()
            .for_each(|t| assert_eq!(t.output[1].value, fee::DUST));
        assert_eq!(
            fee::DUST.to_sat(),
            chain.iter_unsigned_txs().last().unwrap().output[0]
                .value
                .to_sat()
        );

        let total_value = chain
            .iter_unsigned_txs()
            .map(|t| t.output[1].value)
            .sum::<Amount>()
            + chain.iter_unsigned_txs().last().unwrap().output[0].value
            + Amount::from_sat(size);
        assert_eq!(
            ConnectorChain::required_budget(100).to_sat(),
            total_value.to_sat()
        );
        ...
        assert_eq!(
            txs.next().unwrap().output[0].value,
            ConnectorChain::required_budget(99)
        );
        assert_eq!(
            txs.next().unwrap().output[0].value,
            ConnectorChain::required_budget(98)
        );
    }
