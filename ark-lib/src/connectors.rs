...
mod test {
	...
	fn test_budget() {
		...
		for tx in chain.iter_signed_txs(&key) {
			...
			assert_eq!(tx.input[0].witness.len(), INPUT_WEIGHT);
		}
		...
		assert_eq!(fee::DUST.to_sat(), chain.iter_unsigned_txs().last().unwrap().output[0].value.to_sat());
		...
		assert_eq!(ConnectorChain::required_budget(100).to_sat(), total_value.to_sat());
